...
void print_buffer(){
	...
	while(frame!=NULL){
		cout << "(Table id : " << frame->table_id<<", Pagenum : "<<frame->page_num<<
		", Is dirty : "<<frame->is_dirty<< ", Is pinned : "<<(frame->is_pinned)*99999<< ", idx : " << frame->frame_idx << ") == >";
		frame = frame->next;
	}
	cout <<"\n";
}
...
void make_leaf_page(page_t * new_leaf) { // make is_leaf to 1
	if (verbose) cout << "make_leaf_page ===>\n";
	make_internal_page(new_leaf);
	new_leaf->header.is_leaf = 1;
	new_leaf->right_sibling_number = 0;
}
void make_internal_page(page_t * new_page) { // make all of things and is_leaf is 0
